...
# importing analysis dependent helper function
from bilby_tgr.mdr.asimov.flowutils import sort_mdr_by_subtype, fill_in_mdr_specific_metadata
...
class Collector:
    ...
    def _get_pe_result_from_production(self, analysis, corresponding_analysis):
        ...
        if analysis.finished:
            ...
            if str(analysis.pipeline).lower() in ["bilbymdr"]:
    ...
    def _sort_analysis_by_subtype(self, analysis):
        ...
        """
        if analysis.meta["tgr schema section"] == "MDRAnalyses":
            uid = sort_mdr_by_subtype(analysis)
        else:
            uid = 'default'
        return uid
    ...
    def _fill_in_analysis_specific_metadata(self, analysis, corresponding_analysis):
        ...
        if analysis.meta["tgr schema section"] == "MDRAnalyses":
            return fill_in_mdr_specific_metadata(analysis, corresponding_analysis)
        else:
            return dict()
...
class Applicator:
    ...
    def run(self, sid=None):
        ...
        config = utils.bilby_config_to_asimov(config_file)
        ...
        output = utils.deep_update(output, config)
